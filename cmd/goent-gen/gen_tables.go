	...
	for junctionName, referencedTables := range junctionTables {
		for i, refTable := range referencedTables {
			if refTable == info.Name {
				...
				fmt.Fprintf(buf, "\t%s []%s `goe:\"m2m\"` // via %s\n", ToPlural(otherStructName), otherStructName, junctionName)
			}
		}
	}
...
func GenerateModelsFile(buf *bytes.Buffer, tables map[string]*TableInfo,
	...
	thirdPartyImports["\"github.com/azhai/goent\""] = true
	if dbType == "pgsql" {
		thirdPartyImports["\"github.com/azhai/goent/drivers/pgsql\""] = true
	} else {
		thirdPartyImports["\"github.com/azhai/goent/drivers/sqlite\""] = true
	}
	...
	for _, info := range tables {
		for _, col := range info.Columns {
			...
			if goType == "uuid.UUID" {
				thirdPartyImports["\"github.com/google/uuid\""] = true
			}
			if goType == "json.RawMessage" {
				stdImports["\"encoding/json\""] = true
			}
		}
	}
	...
	if len(stdImports) > 0 || len(thirdPartyImports) > 0 {
		...
		if len(thirdPartyImports) > 0 {
			for imp := range thirdPartyImports {
				fmt.Fprintf(buf, "\t%s\n", imp)
			}
		}
		fmt.Fprintf(buf, ")\n\n")
