...
func runIDCompact(args *IDCompactArgs) {
	if args.Gap < 1 {
	...
	if len(args.Tables) == 0 {
		fmt.Fprintln(os.Stderr, "Error: at least one table name is required")
		os.Exit(1)
	}
	...
	for _, table := range args.Tables {
		...
		if err := w.compactTable(args.Gap, args.DryRun); err != nil {
			fmt.Printf("Error compacting table %s: %v\n", table, err)
		}
	}
}

func (w *TableWork) compactTable(gapThreshold int64, dryRun bool) error {
	isPg := w.IsPg()
	var count int64
	var maxID int64
	rows, err := w.db.RawQueryContext(w.ctx, fmt.Sprintf("SELECT COUNT(*), COALESCE(MAX(id), 0) FROM %s", quote(w.Table)))
	if err != nil {
	...
	if totalGap <= gapThreshold {
		...
		if !dryRun {
			if err := w.resetSequence(maxID + 1); err != nil {
	...
	idRows, err := w.db.RawQueryContext(w.ctx, fmt.Sprintf("SELECT id FROM %s ORDER BY id", quote(w.Table)))
	if err != nil {
	...
	fks, err := w.SchemaOps().DiscoverFKs(w.ctx, w.Table)
	if err != nil {
	...
	if isPg {
		if err := w.compactPostgres(oldIDs, segMap, fks); err != nil {
			tx.Rollback()
			return err
		}
	} else {
		if err := w.compactSQLite(oldIDs, segMap, fks); err != nil {
			tx.Rollback()
			return err
		}
	}
	...
	if isPg {
		...
		seqExists, _ := w.SchemaOps().SequenceExists(w.ctx, seqName)
		if seqExists {
			fmt.Printf("  Setting sequence %s to %d...\n", seqName, nextVal)
			if err := w.TxExec(fmt.Sprintf("SELECT setval('%s', %d, false)", seqName, nextVal)); err != nil {
...
func (w *TableWork) resetSequence(nextVal int64) error {
	ops := w.SchemaOps()
	seqName := w.Table + "_id_seq"
...
func (w *TableWork) compactPostgres(oldIDs []int64, segMap map[int64]int64, fks []model.FKRef) error {
	tempTable := "_compact_tmp_" + w.Table
	if err := w.TxExec(fmt.Sprintf("CREATE TEMP TABLE %s (old_id BIGINT, new_id BIGINT)", quote(tempTable))); err != nil {
	...
	for i := 0; i < len(oldIDs); i += batchSize {
		...
		sb.WriteString(fmt.Sprintf("INSERT INTO %s (old_id, new_id) VALUES ", quote(tempTable)))
		for j := i; j < end; j++ {
			if j > i {
				sb.WriteString(", ")
			}
		...
		if err := w.TxExec(sb.String()); err != nil {
	...
	for _, fk := range fks {
		...
		query := fmt.Sprintf("UPDATE %s SET %s = t.new_id FROM %s t WHERE %s = t.old_id%s",
			quote(fk.FromTable), fk.FromColumn, quote(tempTable), fk.FromColumn, nullCond)
		if err := w.TxExec(query); err != nil {
			return fmt.Errorf("updating FK %s.%s: %w", fk.FromTable, fk.FromColumn, err)
		}
	}

	fmt.Printf("  Updating primary key in %s...\n", w.Table)
	query := fmt.Sprintf("UPDATE %s SET id = t.new_id FROM %s t WHERE %s.id = t.old_id", quote(w.Table), quote(tempTable), quote(w.Table))
	if err := w.TxExec(query); err != nil {
		return fmt.Errorf("updating primary key: %w", err)
	}

	w.TxExec("DROP TABLE " + quote(tempTable))
	return nil
}

func (w *TableWork) compactSQLite(oldIDs []int64, segMap map[int64]int64, fks []model.FKRef) error {
	...
	if len(conflictIDs) > 0 {
		...
		for oldID := range conflictIDs {
			tempID := -oldID
			if err := w.TxExec(fmt.Sprintf("UPDATE %s SET id = ? WHERE id = ?", quote(w.Table)), tempID, oldID); err != nil {
			...
			for _, fk := range fks {
				...
				if err := w.TxExec(fmt.Sprintf("UPDATE %s SET %s = ? WHERE %s = ?%s", quote(fk.FromTable), fk.FromColumn, fk.FromColumn, nullCond), tempID, oldID); err != nil {
	...
	for _, oldID := range sortedIDs {
		...
		if err := w.TxExec(fmt.Sprintf("UPDATE %s SET id = ? WHERE id = ?", quote(w.Table)), newID, srcID); err != nil {
		...
		for _, fk := range fks {
			...
			if err := w.TxExec(fmt.Sprintf("UPDATE %s SET %s = ? WHERE %s = ?%s", quote(fk.FromTable), fk.FromColumn, fk.FromColumn, nullCond), newID, srcID); err != nil {
				return fmt.Errorf("updating FK %s.%s: %w", fk.FromTable, fk.FromColumn, err)
			}
		}
	}
