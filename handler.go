...
// GenInsertChanges is an interface for models that can provide insert field-value pairs without reflection.
// It is typically implemented by code generated by goent-gen.
// When available, StateInsert.One uses this interface instead of CollectFields with reflection.
type GenInsertChanges interface {
	// InsertChanges writes non-PK field values into the provided changes map,
	// and returns the PK column name, PK value (if set), and PK field ID (-1 if no PK).
	InsertChanges(table *TableInfo, changes map[*Field]any) (pkName string, pkValue any, pkFid int)
}
