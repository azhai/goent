...
// isColumnExistsInMigrate checks if a column name already exists in any relation or attribute
func isColumnExistsInMigrate(tm *model.TableMigrate, colNames ...string) bool {
...
// resolveTypeName resolves the SQL type name from a Go reflect.Type.
// It handles pointer, slice, array, and custom types.
// This is a unified version that replaces both getTagType and getColumnTypeName.
...
// getTagType resolves type name from a struct field, respecting the `type:` tag override
...
func helperAttributeMigrate(b body) error {
	...
	if utils.HasTagValue(goeTag, "m2o") || utils.HasTagValue(goeTag, "o2o") {
		...
		if table != "" {
			fd := createFieldDescByName(table, migField.Name)
			if fd != nil {
				addRelationByTag(b.migrate.table, fd, migField, goeTag, b.driver, b.fieldId, b.nullable)
				return nil
			}
		}
		return migrateAtt(b)
	...
	// Handle implicit foreign key patterns (e.g., CategoryID)
	...
	if isColumnExistsInMigrate(migTable, fkColName, utils.ToSnakeCase(migField.Name)) {
		return nil
	}
...
// createManyToSomeMigrate creates a ManyToSomeMigrate relation for migration
...
// createOneToSomeMigrate creates a OneToSomeMigrate relation for migration
